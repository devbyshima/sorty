...
public final class TrackListModel {
    ...
    /// dropped - because using Sorty must never cost a listener tracks.
    ///
    ...
    /// Whether this playlist is one Sorty may write over at all - a question
    ...
    /// here, so no sequence of taps in Sorty can remove a track from a
    ...
    public func saveAsNewPlaylist() async {
        ...
        guard let userID = currentUserID else {
            saveStatus = .failed("Sorty doesn't know which account to save to.")
            return
        }
