        ...
        VStack(alignment: .leading, spacing: 8) {
            HStack(spacing: 6) {
                ForEach(ConnectStep.allCases) { each in
                    Capsule()
                        .fill(each <= step ? AnyShapeStyle(SortifyTheme.accent) : AnyShapeStyle(SortifyTheme.raisedSurface))
                        .frame(height: 4)
                }
            }
            Text(step.position)
                ...
                TextField("Client ID", text: $clientID)
                    ...
                    .overlay {
                        RoundedRectangle(cornerRadius: 10)
                            .stroke(
                                clientID.isEmpty
                                    ? SortifyTheme.hairline
                                    : (check.isValid ? SortifyTheme.accent : .orange),
                                lineWidth: 1
                            )
                    }
