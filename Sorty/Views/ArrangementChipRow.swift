...
///
/// The composition is `ArrangementChip.row(for:)` in SortyKit - which five are
...
private struct ChipView: View {
    ...
    var body: some View {
        ...
        .padding(.horizontal, 14)
        .frame(height: 38)
        .foregroundStyle(chip.isActive ? AnyShapeStyle(SortyTheme.onAccent) : AnyShapeStyle(.primary))
        ...
        .background(chip.isActive ? AnyShapeStyle(SortyTheme.accent) : AnyShapeStyle(.clear), in: .capsule)
        .glassEffect(.regular.interactive(), in: .capsule)
