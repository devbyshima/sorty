...
///
/// - **The anchor action is Save, not play.** Sorty has no playback. Save is
...
struct TrackListView: View {
    ...
    ///
    /// Whether to animate at all is `ReorderAnimation.animates` in SortyKit:
    ...
    private var cover: some View {
        StickerCover(images: model.playlist.artworkImages, cornerRadius: 8)
            .frame(maxWidth: 260)
            .shadow(color: SortyTheme.cardShadow(colorScheme), radius: 18, y: 10)
    ...
    /// the reference puts here, and all three are things Sorty knows.
    ///
    /// Every rule in it lives in `PlaylistHeaderText` in SortyKit, including
    ...
    /// `SaveAction` in SortyKit - ADR-0002's split is a rule about what each
    ...
    private func saveGlyph(isArmed: Bool) -> some View {
        ZStack {
            if model.saveStatus == .saving {
                ...
                Image(systemName: "arrow.down.to.line")
                    ...
                    .foregroundStyle(isArmed ? AnyShapeStyle(SortyTheme.onAccent) : AnyShapeStyle(.secondary))
                    .transition(saveGlyphTransition)
            }
        }
        ...
        .glassEffect(
            isArmed
                ? .regular.tint(SortyTheme.accent).interactive()
                : .regular.interactive(),
            in: .circle
        )
...
private struct TrackListRow: View {
    ...
    /// Every word on the row, decided in SortyKit where it is tested.
